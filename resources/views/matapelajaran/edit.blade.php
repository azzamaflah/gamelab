...
<!-- Main content -->
        <div class="card">
            <div class="card-body">
                <form action="{{ route('updateMapel', ['id' => $matapelajaran->id])}}" method="post">
                    @csrf
                    ...
                    <select class="form-control" name="id_jurusan" id="id_jurusan" required="required">
                        @foreach($jurusans as $jurusan)
                            <option value="{{ $jurusan->id }}"{{ $jurusan->id == $matapelajaran->id_jurusan ? 'selected' : '' }}>{{ $jurusan->nama }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <div class="text-right">
                        <a href="{{ route('daftarJurusan') }}" class="btn-outline-secondary mr-2" role="button">Batal</a>
